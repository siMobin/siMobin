...
def get_alltime_languages_card(
    api_key,
    username,
    text_color='5f574f',
    font_family='Calibri',
    ...
    if not data or not data.get('languages'):
        raise ValueError('No all-time language data available.')

    num_langs = parse_number(num_langs, 5)
    top_languages = sorted(data['languages'], key=lambda x: -x['total_seconds'])[:num_langs]

    total_seconds = sum(l['total_seconds'] for l in top_languages)
    max_seconds = max(l['total_seconds'] for l in top_languages) if top_languages else 0

    if heading_type == 'friendly':
        top_lang = top_languages[0]['name'] if top_languages else 'Unknown'
        heading_text = f'I mostly code in {top_lang}'
    else:
        heading_text = 'My Top Languages'
    ...
    chart_width = len(top_languages) * (bar_width + spacing)
    ...
    if chart_type == 'bar':
        ...
        for i, lang in enumerate(top_languages):
            ...
            if not parse_boolean(hide_time, False):
                blocks.append(f'<text font-family="{font_family}" x="{x + bar_width / 2}" y="{y - 4}" font-size="9" text-anchor="middle" fill="#{text_color}">{short_time}</text>')
            blocks.append(f'<text font-family="{font_family}" x="{x + bar_width / 2}" y="{chart_base + 12}" font-weight="bold" font-size="10" text-anchor="middle" fill="#{text_color}">{truncate_label(lang["name"], 10)}</text>')
            if not parse_boolean(hide_percentage, False):
                blocks.append(f'<text font-family="{font_family}" x="{x + bar_width / 2}" y="{chart_base + 24}" font-size="9" text-anchor="middle" fill="#{text_color}">{pct}%</text>')
            bar_blocks.append('\n'.join(blocks))
        y_axis_elements = []
        if has_y_axis:
        ...
        row_height = 24
        bar_max_width = chart_width - left_padding - 110
        bar_blocks = []
        for i, lang in enumerate(top_languages):
            seconds = lang['total_seconds']
            pct = f'{(seconds / total_seconds * 100):.1f}' if total_seconds else '0.0'
            ...
            short_time = format_short_time(seconds)
            blocks = []
            blocks.append(f'<text font-family="{font_family}" x="{75 + left_padding - 8}" y="{y + 9}" font-size="10" text-anchor="end" fill="#{text_color}">{truncate_label(lang["name"], 10)}</text>')
            blocks.append(f'<rect x="{75 + left_padding}" y="{y}" width="{bar_w}" height="12" fill="#{fill}" rx="2" ry="2" />')
            hide_t = parse_boolean(hide_time, False)
            hide_p = parse_boolean(hide_percentage, False)
            if not hide_t and hide_p:
                blocks.append(f'<text font-family="{font_family}" x="{75 + left_padding + bar_w + 6}" y="{y + 9}" font-size="9" fill="#{text_color}">{short_time}</text>')
            if not hide_t and not hide_p:
                blocks.append(f'<text font-family="{font_family}" x="{75 + left_padding + bar_w + 6}" y="{y + 9}" font-size="9" fill="#{text_color}">{short_time}    |    {pct}%</text>')
            if hide_t and not hide_p:
                blocks.append(f'<text font-family="{font_family}" x="{75 + left_padding + bar_w + 6}" y="{y + 9}" font-size="9" fill="#{text_color}">{pct}%</text>')
            bar_blocks.append('\n'.join(blocks))
        height = len(top_languages) * row_height + chart_top + 30
        ...
        angle_step = 2 * math.pi / len(top_languages)
        use_lighten = is_dark_color(bg_color)
        ...
        for i, lang in enumerate(top_languages):
            ...
            points.append({'x': x, 'y': y, 'angle': angle, 'seconds': seconds, 'label': truncate_label(lang['name'], 10), 'pct': pct})
        ...
        for i, lang in enumerate(top_languages):
            ...
            labels.append(f'<text font-family="{font_family}" x="{x}" y="{y}" font-size="10" text-anchor="middle" alignment-baseline="middle" fill="#{text_color}">{truncate_label(lang["name"], 6)}</text>')
        ...
        center_offset_x = ((chart_width + 95) - legend_cols * legend_col_width) / 2

        all_pill_texts = []
        for lang in top_languages:
            t = format_short_time(lang['total_seconds'])
            p = f'{((lang["total_seconds"] / total_seconds) * 100):.1f}%' if total_seconds else '0.0%'
        ...
        if not parse_boolean(hide_legend, False):
            for i, lang in enumerate(top_languages):
                short_time = format_short_time(lang['total_seconds'])
                pct_val = f'{((lang["total_seconds"] / total_seconds) * 100):.1f}%' if total_seconds else '0.0%'
                label = truncate_label(lang['name'], 10)
                ...
                pill_x = x + 70
                ratio = lang['total_seconds'] / max_seconds if max_seconds else 0
                darkness = round(10 + pow(ratio, 1.8) * 90)
        ...
        legend_height = math.ceil(len(top_languages) / legend_cols) * legend_line_height + legend_y_offset
        if not parse_boolean(hide_legend, False):
        ...
        bubble_data = [{'value': l['total_seconds'], 'index': i} for i, l in enumerate(top_languages)]
        ...
        center_offset_x_bubble = ((chart_width + 65) - legend_cols * legend_col_width) / 2

        for i, leaf in enumerate(leaves):
            lang_index = leaf['data']['index']
            lang = top_languages[lang_index]
            seconds = lang['total_seconds']
            pct = f'{(seconds / total_seconds * 100):.1f}' if total_seconds else '0.0'
            short_time = format_short_time(seconds)
            label = truncate_label(lang['name'], 10)
            ...
            if not parse_boolean(hide_legend, False):
                ...
                legend_items.append(f'<text font-family="{font_family}" x="{x + legend_text_offset + 70}" y="{y + 3}" font-size="10" fill="#{text_color}">{short_time} ({pct}%)</text>')

        bubble_group = f'<g transform="translate({x_offset}, 0)">\n' + '\n'.join(elements) + '\n</g>'

        if not parse_boolean(hide_legend, False):
            legend_h = math.ceil(len(top_languages) / legend_cols) * legend_line_height + legend_y_offset_bubble
            height += legend_h
        ...
        for i, lang in enumerate(top_languages):
            ...
            if not hide_outer_label:
                ...
                elements.append(f'<text font-family="{font_family}" font-size="9" fill="#{text_color}"><textPath href="#{day_path_id}" font-family="{font_family}" startOffset="{start_offset}" text-anchor="{anchor}">{label}</textPath></text>')
