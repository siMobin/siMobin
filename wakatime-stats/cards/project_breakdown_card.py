...
def get_project_breakdown_card(
    api_key=None,
    username='',
    text_color='',
    chart_color='',
    ...
    for d in days:
        ...
        project_map = {}
        for p in projects:
            project_map[p['name']] = project_map.get(p['name'], 0) + p['total_seconds']
        project_data_by_day.append({
            'date': date,
            'projects': project_map
        })

    all_projects = list(set(
        name for day_data in project_data_by_day
        for name in day_data['projects']
    ))

    project_series = {}
    for name in all_projects:
        project_series[name] = [
            day_data['projects'].get(name, 0)
            for day_data in project_data_by_day
        ]

    daily_totals = [
        sum(day_data['projects'].values())
        for day_data in project_data_by_day
    ]
    ...
    if heading_type == 'friendly':
        ...
        sorted_projs = sorted(project_totals.items(), key=lambda x: -x[1])
        top_project = sorted_projs[0][0] if sorted_projs else 'various things'
        heading_text = f'This week, I mostly worked on {top_project}'
    else:
    ...
    if chart_type == 'bar':
        ...
        for i, d in enumerate(days):
            ...
            for project in all_projects:
                seconds = project_series[project][i]
                if not seconds:
                    continue
                ...
                y -= segment_height
                day_blocks.append(f'''
            <rect x="{x}" y="{y}" width="{bar_width}" height="{segment_height}" fill="#{project_colors[project]}" rx="2" ry="2">
              <title>{truncate_label(project, 20)}: {format_short_time(seconds)}</title>
            </rect>
          ''')
        ...
        legend_cols = 2
        max_label_length = max(len(p) for p in all_projects) if all_projects else 0
        avg_char_width = 6.5
        ...
        if not hide_legend:
            ...
            <circle cx="{lx}" cy="{ly - 4}" r="5" fill="#{project_colors[project]}" />
            <text x="{lx + 12}" y="{ly}" font-size="10" fill="#{text_color}">{truncate_label(project, 20)}</text>
          ''')

        legend_height_total = 0 if hide_legend else math.ceil(len(all_projects) / legend_cols) * legend_line_height + legend_y_offset
        ...
        for project in all_projects:
            points = []
            for i, seconds in enumerate(project_series[project]):
            ...
            for p in points:
                elements.append(f'<circle cx="{p["x"]}" cy="{p["y"]}" r="2" fill="#{color}"><title>{truncate_label(project, 20)}: {format_short_time(p["seconds"])}</title></circle>')
        ...
        if not hide_legend:
          ...
          ''')
            ...
            legend_height_total = math.ceil(len(all_projects) / legend_cols) * legend_line_height + legend_y_offset
      ...
      '''
            ...
            <circle cx="{p["x"]}" cy="{p["y"]}" r="2.2" fill="#{color}">
              <title>{truncate_label(project, 20)}: {short_time}</title>
            </circle>
        ...
        for project in all_projects:
            ...
            color = project_colors[project]
            project_polygons.append(f'''
          <polygon points="{polygon_points}" fill="#{color}" fill-opacity="0.2" stroke="#{color}" stroke-width="2" />
          {"".join(f'<circle cx="{p["x"]}" cy="{p["y"]}" r="2.2" fill="#{color}"><title>{truncate_label(project, 20)}: {format_short_time(p["seconds"])}</title></circle>' for p in points)}
        ''')
        ...
        legend_cols_radar = 1
        max_label_length_radar = max(len(p) for p in all_projects) if all_projects else 0
        ...
        svg_width_radar2 = max(chart_width, 300) + 25
        ...
        for proj in all_projects:
            project_totals[proj] = sum(project_series[proj])
        ...
        if not hide_legend:
            for idx, project in enumerate(all_projects):
                ...
                value_seconds = project_totals[project]
                raw_color = project_colors.get(project, chart_color)
            ...
            <text x="{lx}" y="{ly}" font-size="10" font-weight="bold" fill="#{text_color}">{truncate_label(project, 20)}</text>
        ...
        legend_height_radar = math.ceil(len(all_projects) / legend_cols_radar) * legend_line_height_radar + legend_y_offset_radar
        if not hide_legend:
            height = chart_base + radar_padding + legend_height_radar + 15

        chart_blocks = grid + axis_lines + project_polygons + labels + radar_y_axis + legend_items_radar
